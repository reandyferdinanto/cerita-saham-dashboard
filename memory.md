...
- `/admin?tab=breakdown` provides an admin-only daily candle breakdown chart with price + MA5/MA10/MA20/MA60; clicked candles show price/high/low rows for 5 trading candles before/after, can be saved/deleted in a local admin high-low list, and saved row dates reopen the matching ticker/date in the chart.
