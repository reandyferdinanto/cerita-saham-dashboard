...
const AccDistFluxChart = dynamic(
  () => import("./AccDistFluxChart"),
  { ssr: false }
);
...
function BotViewContent({ tickerParam }: { tickerParam: Promise<{ ticker: string }> }) {
  ...
  const fetchFlow = useCallback(async () => {
    try {
      const tickerJK = decodedTicker.toUpperCase().endsWith(".JK") ? decodedTicker.toUpperCase() : `${decodedTicker.toUpperCase()}.JK`;
      const res = await fetch(`/api/stocks/flow/${encodeURIComponent(tickerJK)}?days=60`);
      const data = await res.json();
      if (Array.isArray(data)) setFlowData(data);
    } catch {}
  }, [decodedTicker]);
  ...
  useEffect(() => {
    ...
    fetchFlow();
  }, [fetchQuote, fetchHistory, fetchFlow]);
  ...
  return (
    <div className="fixed inset-0 bg-black p-8 w-[1000px] h-[1250px] text-silver-100 flex flex-col gap-6 z-[9999] overflow-hidden" id="capture-area">
      ...
      <div className="flex justify-between items-end border-b-2 border-white/10 pb-5">
        <div>
      ...
      <div className="grid grid-cols-4 gap-6">
        {[
          ...
          <div key={s.label} className="bg-white/5 border-2 border-white/10 p-4 rounded-[32px] flex flex-col items-center justify-center">
      ...
      <div className="flex-1 bg-white/5 border-2 border-white/10 rounded-[48px] p-8 relative overflow-hidden">
        ...
        {history.length > 0 ? (
          <CandlestickChart data={history} height={500} />
        ) : (
      ...
      {flowData.length > 0 && (
        <div className="bg-white/5 border-2 border-white/10 rounded-[32px] px-8 py-5">
          <div className="flex items-center gap-6 mb-2">
            <span className="text-[11px] font-black uppercase tracking-[0.15em] text-silver-400">Acc/Dist Flux</span>
            <span className="flex items-center gap-1.5 text-[10px] font-bold text-emerald-400">
              <span className="inline-block w-3 h-3 rounded-sm bg-emerald-500"></span> Foreign Acc
            </span>
            <span className="flex items-center gap-1.5 text-[10px] font-bold text-red-400">
              <span className="inline-block w-3 h-3 rounded-sm bg-red-500"></span> Foreign Dist
            </span>
            <span className="flex items-center gap-1.5 text-[10px] font-bold text-cyan-400">
              <span className="inline-block w-3 h-3 rounded-sm bg-cyan-500"></span> Domestic Acc
            </span>
            <span className="flex items-center gap-1.5 text-[10px] font-bold text-orange-400">
              <span className="inline-block w-3 h-3 rounded-sm bg-orange-500"></span> Domestic Dist
            </span>
          </div>
          <AccDistFluxChart data={flowData} height={150} />
        </div>
      )}

      <div className="flex justify-between items-center border-t-2 border-white/5 pt-4">
